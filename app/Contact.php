...
class Contact extends Authenticatable
{
    ...
    public static function contactDropdown($business_id, $exclude_default = false, $prepend_none = true, $append_id = true)
    {
        ...
        if ($append_id) {
            $query->select(
                DB::raw("IF(contacts.mobile IS NULL OR contacts.mobile='', CONCAT(contacts.name, ' ', COALESCE(contacts.supplier_business_name, '')), CONCAT(name, ' ', COALESCE(contacts.supplier_business_name, ''), ' (', contacts.mobile, ')')) AS supplier"),
                'contacts.id'
                    );
        } else {
            $query->select(
    ...
    public static function customersDropdown($business_id, $prepend_none = true, $append_id = true)
    {
        ...
        if ($append_id) {
            $all_contacts->select(
                DB::raw("IF(contacts.mobile IS NULL OR contacts.mobile='', CONCAT(contacts.name, ' ', COALESCE(contacts.supplier_business_name, '')), CONCAT(name, ' ', COALESCE(contacts.supplier_business_name, ''), ' (', contacts.mobile, ')')) AS customer"),
                'contacts.id'
                );
        } else {
