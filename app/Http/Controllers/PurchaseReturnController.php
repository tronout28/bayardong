        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($purchases_returns)
                ->addColumn('action', function ($row) {
					...
					// hero70411 - them lenh xem, sua, xoa
                    if (auth()->user()->can('purchase.view')) {
						$return_id = ! empty($row->return_parent_id) ? $row->return_parent_id : $row->id;
                        $html .= '<li><a href="#" data-href="'.action([\App\Http\Controllers\PurchaseReturnController::class, 'show'], [$return_id]).'" class="btn-modal" data-container=".view_modal"><i class="fas fa-eye" aria-hidden="true"></i>'.__('messages.view').'</a></li>';
                    }
					if (auth()->user()->can('purchase.update')) {
						if (! empty($row->return_parent_id)) {
							$html .= '<li><a href="'.action([\App\Http\Controllers\PurchaseReturnController::class, 'add'], $row->return_parent_id).'" ><i class="glyphicon glyphicon-edit"></i>'.__('messages.edit').'</a></li>';
						} else {
							$html .= '<li><a href="'.action([\App\Http\Controllers\CombinedPurchaseReturnController::class, 'edit'], $row->id).'" ><i class="glyphicon glyphicon-edit"></i>'.__('messages.edit').'</a></li>';
						}
					}
					if (auth()->user()->can('purchase.delete')) {
						$html .= '<li><a href="'.action([\App\Http\Controllers\PurchaseReturnController::class, 'destroy'], $row->id).'" class="delete_purchase_return" ><i class="fa fa-trash"></i>'.__('messages.delete').'</a></li>';
					}
                ...
                ->editColumn('name', function ($row) {
                    ...
                    return '<a href="'.action([\App\Http\Controllers\ContactController::class, 'show'], $row->supplier_id).'">'.$name.'</a>';
                })
                ->editColumn(
                    'payment_status',
					// hero70411 - sua giao dien nut bam
                ...
                ->rawColumns(['action', 'final_total', 'name', 'payment_status', 'parent_purchase', 'payment_due'])
                ->make(true);
        }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $return_transaction_data = [
				...
				// hero70411 - sua loi khong luu So chung tu
