...
class ReportController extends Controller
{
        ...
        if ($request->ajax()) {
            $contacts = Contact::where('contacts.business_id', $business_id)
                ...
                ->select(
                    ...
                    DB::raw("SUM(IF(t.type = 'ledger_discount' AND t.sub_type='sell_discount', final_total, 0)) as total_ledger_discount_sell"),
                    DB::raw("SUM(IF(t.type = 'ledger_discount' AND t.sub_type='purchase_discount', final_total, 0)) as total_ledger_discount_purchase"),
			...
			// hero70411 - fix loi Bao cao Nha cung cap & Khach hang
            return Datatables::of($contacts)
                ->editColumn('name', function ($row) {
                    ...
                    return '<a href="'.action([\App\Http\Controllers\ContactController::class, 'show'], [$row->id]).'">'.$name.'</a>';
                })
                ->editColumn(
                    'total_purchase',
                ...
                ->addColumn(
                    'opening_balance',
                    '<span class="opening_balance" data-orig-value="{{$opening_balance}}">@format_currency($opening_balance)</span>'
                )
                ->addColumn('due', function ($row) {
                    ...
                    $due = ($row->total_invoice - $row->invoice_received - $total_ledger_discount_sell) - ($row->total_purchase - $row->purchase_paid - $total_ledger_discount_purchase) - ($row->total_sell_return - $row->sell_return_paid) + ($row->total_purchase_return - $row->purchase_return_received) + $row->opening_balance - $row->opening_balance_paid;
                ...
                ->rawColumns(['name', 'total_purchase', 'total_purchase_return', 'total_invoice', 'total_sell_return', 'opening_balance', 'due'])
                ->make(true);
        }
    ...
    public function getTaxDetails(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $datatable->editColumn(
				...
				->editColumn('contact_name', '<a href="{{action(\'App\Http\Controllers\ContactController@show\', [$contact_id])}}">{{$contact_name}} @if(!empty($supplier_business_name)) {{$supplier_business_name}}@endif </a>')
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
				...
				->editColumn('supplier', function ($row) {
					...
					return '<a href="'.action([\App\Http\Controllers\ContactController::class, 'show'], $row->supplier_id).'">'.$supplier.'</a>';
				})
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
				...
				->editColumn('customer', function ($row) {
					...
					if (! empty($row->supplier_business_name)) {
                        $customer .= ' '.$row->supplier_business_name;
                    }
					return '<a href="'.action([\App\Http\Controllers\ContactController::class, 'show'], $row->customer_id).'">'.$customer.'</a>';
				})
            ...
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                ...
                ->select(
                    DB::raw("IF(transaction_payments.transaction_id IS NULL, 
                                ...
                                (SELECT CONCAT(c.name, ' ', COALESCE(c.supplier_business_name, '')) FROM transactions as ts JOIN
                                    contacts as c ON ts.contact_id=c.id
                                    WHERE ts.id=t.id 
                                )
                            ) as supplier"),
					DB::raw("IF(transaction_payments.transaction_id IS NULL, 
                                (SELECT c.id FROM transactions as ts
                                JOIN contacts as c ON ts.contact_id=c.id 
                                WHERE ts.id=(
                                        SELECT tps.transaction_id FROM transaction_payments as tps
                                        WHERE tps.parent_id=transaction_payments.id LIMIT 1
                                ...
                                (SELECT c.id FROM transactions as ts JOIN
                                    contacts as c ON ts.contact_id=c.id
                                    WHERE ts.id=t.id 
                                )
                            ) as supplier_id"),
    ...
    public function getProfit($by = null)
    {
        ...
        if ($by == 'customer') {
            ...
            ->addSelect('CU.name as customer', 'CU.supplier_business_name', 'CU.id as customer_id')
            ...
            return Datatables::of($query)
				...
				->editColumn('supplier', function ($row) {
					...
					if (! empty($row->supplier_id)) {
						$html = '<a href="'.action([\App\Http\Controllers\ContactController::class, 'show'], $row->supplier_id).'">'.$supplier.'</a>';
					}
					return $html;
				})
				->editColumn('customer', function ($row) {
					...
					if (! empty($row->customer_business_name)) {
                        $customer .= ' '.$row->customer_business_name;
                    }
					$html = $customer;
					if (! empty($row->customer_id)) {
						$html = '<a href="'.action([\App\Http\Controllers\ContactController::class, 'show'], $row->customer_id).'">'.$customer.'</a>';
					}
					return $html;
				})
        ...
        if ($request->ajax()) {
            ...
            return $datatable->addColumn('taxable_value', function ($row) {
                ...
                ->editColumn('tax_percent', '@if(!empty($tax_percent)){{@num_format($tax_percent)}}% @endif
                    ')
                ->editColumn('customer', '<a href="{{action(\'App\Http\Controllers\ContactController@show\', [$customer_id])}}">{{$customer}} @if(!empty($supplier_business_name)) {{$supplier_business_name}}@endif </a>')
                ->rawColumns($raw_cols)
                ->make(true);
        }
    ...
    public function gstPurchaseReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return $datatable->addColumn('taxable_value', function ($row) {
                ...
                ->editColumn('tax_percent', '@if(!empty($tax_percent)){{@num_format($tax_percent)}}% @endif
                    ')
                ->editColumn('supplier', '<a href="{{action(\'App\Http\Controllers\ContactController@show\', [$supplier_id])}}">{{$supplier}} @if(!empty($supplier_business_name)) {{$supplier_business_name}}@endif </a>')
                ->rawColumns($raw_cols)
                ->make(true);
        }
