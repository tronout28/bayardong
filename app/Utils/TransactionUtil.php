...
class TransactionUtil extends Util
{
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($il->show_customer == 1) {
            ...
            if ($receipt_printer_type != 'printer') {
                $output['customer_info'] .= $customer->full_name_with_business;
                if (! empty($customer->contact_address)) {
                    $output['customer_info'] .= '<br><b>'.__('business.address').'</b>: '.$customer->contact_address;
                }
                $output['customer_info'] .= '<br><b>'.__('contact.mobile').'</b>: '.$customer->mobile;
    ...
    public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
        ...
        $previous_transaction_sums = $this->__transactionQuery($contact_id, $start, null, $location_id)
                ->select(
                    ...
                    DB::raw("SUM(IF(type = 'ledger_discount' AND sub_type='sell_discount', final_total, 0)) as total_sell_discount"),
                    DB::raw("SUM(IF(type = 'ledger_discount' AND sub_type='purchase_discount', final_total, 0)) as total_purchase_discount")
                )->first();
        ...
        $total_prev_invoice = $previous_transaction_sums->total_invoice - $previous_transaction_sums->total_purchase - $previous_transaction_sums->total_sell_return + $previous_transaction_sums->total_purchase_return - $previous_transaction_sums->total_opening_balance + $previous_transaction_sums->total_sell_discount - $previous_transaction_sums->total_purchase_discount;

        $beginning_balance = $total_prev_paid - $total_prev_invoice;
        ...
        foreach ($transactions as $transaction) {
            if ($transaction->type == 'opening_balance') {
				...
				$temp_array = [
					...
					'total' => '',
					'payment_method' => '',
					'debit' => $opening_balance < 0 ? abs($opening_balance) : '',
					'credit' => $opening_balance > 0 ? abs($opening_balance) : '',
					'others' => '',
					'transaction_id' => $transaction->id,
					'transaction_type' => $transaction->type,
				];
            ...
            if ($transaction->sub_type == 'purchase_discount') {
                $ledger_discount -= $transaction->final_total;
            }
        ...
        foreach ($payments as $payment) {
            ...
            if (in_array($contact->type, ['customer', 'both']) && $payment->is_advance == 0 && empty($payment->transaction_id) && $payment->payment_type == 'debit') {
                $total_reverse_payment += $payment->amount;
            }
            if (in_array($contact->type, ['supplier', 'both']) && $payment->is_advance == 0 && empty($payment->transaction_id) && $payment->payment_type == 'credit') {
                $total_reverse_payment += $payment->amount;
            }
            ...
            $ledger[] = [
                ...
                'credit' => (in_array($payment->transaction_type, ['sell', 'purchase_return', 'opening_balance']) || ($payment->is_advance == 1 && $contact->type == 'customer')) && $payment->is_return == 0 || $payment->payment_type == 'credit' ? $payment->amount : '',
                'others' => $note,
            ];
        }
        ...
        $curr_balance = $total_purchase - $total_invoice + $total_paid + $beginning_balance + $opening_balance;
        ...
        //$total_opening_bal = $beginning_balance + $opening_balance;
        if ($format != 'format_2') {
            ...
            $ledger = array_merge([[
                ...
                'total' => '',
                'payment_method' => '',
                'debit' => $beginning_balance < 0 ? abs($beginning_balance) : '',
                'credit' => $beginning_balance > 0 ? abs($beginning_balance) : '',
                'others' => '',
                'final_total' => abs($beginning_balance),
                'total_due' => 0,
                'due_date' => null,
            ]], $ledger);
        }
    ...
    public function getEmailAttachmentForGivenTransaction($business_id, $transaction_id, $is_email_attachment)
    {
        ...
        $mpdf->SetTitle(__('lang_v1.invoices').' - ' . $receipt_details->invoice_no.'.pdf');
        $mpdf->WriteHTML($body);
    ...
    public function getPurchaseOrderPdf($business_id, $transaction_id)
    {
        ...
        $mpdf->showWatermarkText = true;
        $mpdf->SetTitle(__('lang_v1.purchase_order').' - ' . $purchase->ref_no.'.pdf');
        $mpdf->WriteHTML($body);
