...
class Util
{
    ...
    public function getContactDue($contact_id, $business_id = null)
    {
        $query = Contact::where('contacts.id', $contact_id)
            ...
            ->whereIn('t.type', ['sell', 'sell_return', 'opening_balance', 'ledger_discount', 'purchase', 'purchase_return'])
            ->select(
        ...
        $due = ($contact_payments->total_invoice - $contact_payments->total_paid - $contact_payments->total_ledger_discount_sell) - ($contact_payments->total_purchase - $contact_payments->purchase_paid - $contact_payments->total_ledger_discount_purchase) - ($contact_payments->total_sell_return - $contact_payments->sell_return_paid) + ($contact_payments->total_purchase_return - $contact_payments->purchase_return_paid) + $contact_payments->opening_balance - $contact_payments->opening_balance_paid;
