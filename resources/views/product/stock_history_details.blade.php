		...
		<table class="table table-slim" id="stock_history_table">
			...
			@forelse($stock_history as $history)
				<tr>
					...
					<td>
						@if(!empty($history['contact_id']))<a href="{{action([\App\Http\Controllers\ContactController::class, 'show'], $history['contact_id'])}}">@endif
						{{$history['contact_name'] ?? '--'}}
						{{$history['supplier_business_name'] ?? ''}}
						@if(!empty($history['contact_id']))</a>@endif
					</td>
				</tr>
			@empty
