...
<div class="modal-body">
  ...
  <div class="row invoice-info">
    <div class="col-sm-4 invoice-col">
      <b>@lang('purchase.supplier'):</b> {{$purchase->contact->full_name_with_business}}
      <address>
        <b>@lang('business.address'):</b> {!! $purchase->contact->contact_address !!}
        @if(!empty($purchase->contact->tax_number))
          <br><b>@lang('contact.tax_no'):</b> {{$purchase->contact->tax_number}}
        @endif
        @if(!empty($purchase->contact->mobile))
          <br><b>@lang('contact.mobile'):</b> {{$purchase->contact->mobile}}
        @endif
        @if(!empty($purchase->contact->email))
          <br><b>@lang('business.email'):</b> {{$purchase->contact->email}}
        @endif
      </address>
    ...
    <div class="col-sm-4 invoice-col">
      <b>@lang('business.business'):</b>
      <address>
        {{ $purchase->business->name }} - {{ $purchase->location->name }}
        ...
        <table class="table bg-gray">
          <thead>
            <tr class="bg-green">
              ...
              <th class="text-right">@lang('purchase.unit_cost_before_tax')</th>
              @if($purchase->type != 'purchase_order')
              <th class="no-print text-right">@lang('purchase.unit_selling_price')</th>
          ...
          @foreach($purchase->purchase_lines as $purchase_line)
            <tr>
              ...
              <td class="text-right"><span class="display_currency" data-currency_symbol="true">{{ $purchase_line->purchase_price }}</span></td>
              @if($purchase->type != 'purchase_order')
              @php
              ...
              @endphp
              <td class="no-print text-right"><span class="display_currency" data-currency_symbol="true">{{$sp}}</span></td>
        ...
        <table class="table">
            ...
            <td class="text-right">
                @if(!empty($purchase_taxes))
                  @foreach($purchase_taxes as $k => $v)
                    <strong>{{$k}}:</strong>&nbsp;<span class="display_currency pull-right" data-currency_symbol="true">{{ $v }}</span><br>
                  @endforeach
                @else
          ...
          @if( !empty( $purchase->shipping_charges ) )
            <tr>
              ...
              <td><b>(+)</b></td>
              <td><span class="display_currency pull-right" data-currency_symbol="true">{{ $purchase->shipping_charges }}</span></td>
            </tr>
          @endif
          @if( !empty( $purchase->additional_expense_value_1 )  && !empty( $purchase->additional_expense_key_1 ))
            <tr>
              ...
              <td><b>(+)</b></td>
              <td><span class="display_currency pull-right" data-currency_symbol="true">{{ $purchase->additional_expense_value_1 }}</span></td>
            </tr>
          @endif
          @if( !empty( $purchase->additional_expense_value_2 )  && !empty( $purchase->additional_expense_key_2 ))
            <tr>
              ...
              <td><b>(+)</b></td>
              <td><span class="display_currency pull-right" data-currency_symbol="true">{{ $purchase->additional_expense_value_2 }}</span></td>
            </tr>
          @endif
          @if( !empty( $purchase->additional_expense_value_3 )  && !empty( $purchase->additional_expense_key_3 ))
            <tr>
              ...
              <td><b>(+)</b></td>
              <td><span class="display_currency pull-right" data-currency_symbol="true">{{ $purchase->additional_expense_value_3 }}</span></td>
            </tr>
          @endif
          @if( !empty( $purchase->additional_expense_value_4 ) && !empty( $purchase->additional_expense_key_4 ))
            <tr>
              ...
              <td><b>(+)</b></td>
              <td><span class="display_currency pull-right" data-currency_symbol="true">{{ $purchase->additional_expense_value_4 }}</span></td>
            </tr>
          @endif
          <tr>
