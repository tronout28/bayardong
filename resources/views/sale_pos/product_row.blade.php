...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	<td>
		...
		@endphp
		<img src="@if(!empty($product->media) && count($product->media) > 0)
	...
	<td>
		...
		@if(!empty($sub_units) && count($sub_units) > 0)
			<br>
	...
	@if(!empty($is_direct_sell))
		...
		<td @if(!$edit_discount) class="hide" @endif>
			{!! Form::select("products[$row_count][line_discount_type]", ['fixed' => __('lang_v1.fixed'), 'percentage' => __('lang_v1.percentage')], $discount_type , ['class' => 'form-control row_discount_type']); !!}<br>
