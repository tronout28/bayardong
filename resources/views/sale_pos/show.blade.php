<div class="modal-dialog modal-xl no-print" role="document">
  <div class="modal-content">
    ...
    <h4 class="modal-title" id="modalTitle"> @lang('sale.sell_details') (<b>@if($sell->type == 'sales_order') @lang('restaurant.order_no') @else @lang('sale.invoice_no') @endif :</b> {{ $sell->invoice_no }})</h4>
	</div>
<div class="modal-body">
    ...
    <div class="row">
      ...
      <div class="@if(!empty($export_custom_fields)) col-sm-3 @else col-sm-4 @endif">
        <b>{{ __('lang_v1.customers') }}:</b> {{$sell->contact->full_name_with_business}}
		<address>
        @if(!empty($sell->billing_address()))
          <b>@lang('lang_v1.billing_address'):</b> {{$sell->billing_address()}}
        @else
          <b>@lang('business.address'):</b> {!! $sell->contact->contact_address !!}
          ...
          @if($sell->contact->email)
            <br>
              <b>{{__('business.email')}}:</b> {{ $sell->contact->email }}
          @endif
        @endif
        </address>
      </div>
...
<div class="modal-footer">
    @if($sell->type != 'sales_order')
      <a href="#" class="print-invoice tw-dw-btn tw-dw-btn-success tw-text-white hover:tw-text-white" data-href="{{route('sell.printInvoice', [$sell->id])}}?package_slip=true"><i class="fas fa-file-alt" aria-hidden="true"></i> @lang("lang_v1.packing_slip")</a>
    @endif
    @can('print_invoice')
      <a href="#" class="print-invoice tw-dw-btn tw-dw-btn-primary tw-text-white hover:tw-text-white" data-href="{{route('sell.printInvoice', [$sell->id])}}"><i class="fa fa-print" aria-hidden="true"></i> @lang("lang_v1.print_invoice")</a>
    @endcan
